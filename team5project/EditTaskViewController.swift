...
//  team5project
//
//  Created by Nicole Estabrook on 12/5/22.
//
...
class EditTaskViewController: UIViewController, UITextFieldDelegate {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        var recurringTaskList = retrieveRecurringTasks()
        var oneTimeTaskList = retrieveOneTimeTasks()
        if editedTaskSection == 0 {
            editedTask = recurringTaskList[editedTaskIndex]
            taskDuration = editedTask.value(forKey: "duration") as? Int
            taskFrequency = editedTask.value(forKey: "frequency") as? Int
            taskTitle.text = editedTask.value(forKey: "title") as? String
            freqDateTextField.text = String(taskFrequency)
            durationTextField.text = String(taskDuration)
        }
        if editedTaskSection == 1 {
            editedTask = oneTimeTaskList[editedTaskIndex]
            recurringToggle.setOn(false, animated: true)
            taskDuration = editedTask.value(forKey: "duration") as? Int
            taskTitle.text = editedTask.value(forKey: "title") as? String
            freqDateTextField.text = formatDate(date: editedTask.value(forKey: "dueDate") as! Date)
            durationTextField.text = String(taskDuration)
            recurLabel.text = "Due Date"
        }
    ...
    @IBAction func saveButtonPressed(_ sender: Any) {
        if editedTaskSection == 0 {
            ...
            let commit = tasks[editedTaskIndex]
            context.delete(commit)
            saveContext()
        }
        if editedTaskSection == 1 {
            var tasks = retrieveOneTimeTasks()
            let commit = tasks[editedTaskIndex]
            context.delete(commit)
            saveContext()
        }
            ...
            if let durationtext = durationTextField.text,!durationtext.isEmpty {
                ...
                if recurringToggle.isOn {
                        ...
                        if freqDouble != -999 {
                            if durationInt != -999 {
                                ...
                                let controller = UIAlertController(
                                    title: "Task updated",
                                    message: "Press \"OK\" to return to task list",
                                    preferredStyle: .alert)
                                let continueButton = UIAlertAction(
                                    title: "OK",
                                    style: .default,
                                    handler: { action in self.taskTitle.text = ""; self.freqDateTextField.text = ""; self.durationTextField.text = String(self.taskDuration);
                    ...
                    if let dateText = dateFormatter.date(from:freqDateTextField.text!) {
                        if durationInt != -999 {
                            storeOneTimeTask(title:taskName!, dueDate:dateText, duration:durationInt)
                            let controller = UIAlertController(
                                title: "Task updated",
                                message: "Press \"OK\" to return to task list",
                                preferredStyle: .alert)
    ...
    func updateRecurringTask(title:String, frequency:Int, duration:Int) {
        
        editedTask.setValue(title, forKey: "title")
        editedTask.setValue(frequency, forKey: "frequency")
        editedTask.setValue(duration, forKey: "duration")
        saveContext()
    }
    ...
    func updateOneTimeTask(title:String, dueDate:Date, duration:Int) {
        
        editedTask.setValue(title, forKey: "title")
        editedTask.setValue(dueDate, forKey: "dueDate")
        editedTask.setValue(duration, forKey: "duration")
        saveContext()
    }
